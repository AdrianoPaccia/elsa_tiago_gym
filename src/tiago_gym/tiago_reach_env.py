...
'''
def check_registration(env_id):
    all_envs = envs.registry.all()
    env_ids = [env_spec.id for env_spec in all_envs]
    if env_id in env_ids: 
        print(env_id, ' is registered!')
    else:
        print(env_id, ' is NOT registered!')
...
class TiagoReachEnv(tiago_env.TiagoEnv):
    ...
    def __init__(self):
        ...
        self.is_multimodal = rospy.get_param('multimodal')
    ...
    def _get_obs(self):
        ...
        if self.is_multimodal:
            observations = [
                self.current_img, #(480, 640, 3)
                np.array(self.stored_arm_state[:3]) #(3)
            ]
        else:
    ...
    def _is_done(self, observations):
        ...
        """
        if self.action_count >= max_episode_steps or rospy.is_shutdown():
